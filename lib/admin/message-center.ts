...
export async function messageLibraryPage(db: D1Database, user: AdminUser, path: string, env: Parameters<typeof csrfToken>[0], canWrite = true) {
  ...
  const cards = templates.map((template, index) => {
    ...
    return `<details class="message-template"><summary class="message-template-header"><h2>${esc(template.title)}</h2><button type="button" class="copy-message" data-copy-source="${id}">Copy message</button></summary><div class="message-template-body">${owner ? `<form method="post" action="${path}"><input type="hidden" name="csrf" value="${esc(csrf)}"><input type="hidden" name="action" value="message_save"><input type="hidden" name="template_key" value="${esc(template.template_key)}"><label>Template name<input name="title" maxlength="120" value="${esc(template.title)}" required></label><label>Message<textarea id="${id}" name="body" maxlength="8000" required>${esc(template.body)}</textarea></label><div class="actions"><button type="submit">Save template</button></div></form><form method="post" action="${path}"><input type="hidden" name="csrf" value="${esc(csrf)}"><input type="hidden" name="action" value="message_reset"><input type="hidden" name="template_key" value="${esc(template.template_key)}"><div class="actions"><button type="submit">Reset to default</button></div></form>` : `<textarea id="${id}" readonly aria-label="${esc(template.title)} message">${esc(template.body)}</textarea>`}</div></details>`;
  }).join("");
  return page("Message Library", `<p class="notice">Use this library for customer communication. Copy a message, review it, then paste it into Messenger, WhatsApp, or email. ${owner ? "You are the owner, so you can edit and reset templates." : "Only the owner can edit these templates."}</p><div class="message-center">${cards}</div><script>document.querySelectorAll('.copy-message').forEach((button)=>button.addEventListener('click',async(event)=>{event.preventDefault();event.stopPropagation();const textarea=document.getElementById(button.dataset.copySource)||button.closest('.message-template').querySelector('textarea');if(!textarea)return;textarea.select();try{await navigator.clipboard.writeText(textarea.value);button.textContent='Copied!'}catch{document.execCommand('copy');button.textContent='Copied!'}setTimeout(()=>button.textContent='Copy message',1600)}))</script><style>.message-center{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:20px}.message-template{margin:0;padding:0}.message-template>summary{list-style:none;cursor:pointer;padding:18px 24px}.message-template>summary::-webkit-details-marker{display:none}.message-template>summary::after{content:"›";position:absolute;right:24px;font-size:1.35rem;transition:transform .15s ease}.message-template[open]>summary::after{transform:rotate(90deg)}.message-template-header{display:flex;align-items:center;justify-content:space-between;gap:16px;position:relative;padding-right:52px}.message-template-header h2{margin:0}.message-template-body{border-top:1px solid #c6dce9;padding:0 24px 24px}.message-template textarea{min-height:250px;white-space:pre-wrap}.message-template form textarea{min-height:250px}.message-template form label{display:block;margin-top:12px}.copy-message{white-space:nowrap}@media(max-width:760px){.message-center{grid-template-columns:1fr}.message-template-header{align-items:flex-start;flex-direction:column}}</style>`, user.name, 200, {}, canWrite);
}
