...
export async function dashboard(
  ...
  const [unpaid, missingCosts, followUp] = await Promise.all([
    db
      .prepare(
        `SELECT i.id,i.invoice_number,c.full_name,s.tracking_number,i.due_at,
        ...
        ORDER BY i.due_at IS NULL,i.due_at,i.created_at
        LIMIT 25`,
      )
      .all<RecordData>(),
    ...
    canWrite
      ? db
          .prepare(
            `SELECT s.id,s.tracking_number,c.full_name,s.status
            FROM shipments s
            JOIN customers c ON c.id=s.customer_id
            WHERE s.status NOT IN ('Delivered','Cancelled')
            AND s.nihao_cost IS NULL
            ORDER BY s.updated_at DESC,s.id
            LIMIT 25`,
          )
          .all<RecordData>()
      : Promise.resolve({
          results: [] as RecordData[],
        }),
  ...
  return page(
    ...
    `
      <section>
        ...
        ${
          note
            ? `<p style="white-space:pre-wrap;color:#c62828;font-weight:600;">${esc(
                note,
              )}</p>
               <p class="muted">
                 Last updated ${esc(
            ...
            : `<p style="color:#c62828;">
                 No follow-up note has been added.
               </p>`
        }
      ...
      <p class="muted">
        Customer and shipment records for the new admin.${
          canWrite
            ? ` Public tracking is managed in the <a href="/admin/tracking-editor">tracking editor</a> during this phase.`
            : ""
        }
      </p>
            ...
            ? `
              ...
              ${
                canWrite && missingRows.length
                  ? `<h3>
                       Shipments missing Nihao cost (${missingRows.length})
                     </h3>
      ...
      ${
        canWrite
          ? `<section>
               ...
               <p>
                 <strong>
                   ${esc(pesos(margin?.margin))}
                 </strong>
                 across ${esc(
                   margin?.costed ?? 0,
                 )} shipments with Ni Hao cost entered.
               </p>

               <p>
                 ${
                   Number(margin?.shipments ?? 0) -
                   Number(margin?.costed ?? 0)
                 }
                 shipments still need a cost.
                 Excludes cancelled shipments,
                 delivery charges and business expenses.
               </p>

               <a href="/admin/finance">
                 Review charges and costs
               </a>
             </section>`
          : ""
      }
