...
export async function leadsPage(request: Request, envOverride?: AdminEnv): Promise<Response> {
  try {
    ...
    return page("Leads", `<section><h2>Website calculator inquiries <span class="notice">New Leads: ${esc(count?.total ?? 0)}</span></h2><form method="get" class="search"><label>Search name, phone, or email<input name="q" value="${esc(q)}"></label><label>Service<select name="service"><option value="">All</option><option${service === "Sea Freight" ? " selected" : ""}>Sea Freight</option><option${service === "Air Freight" ? " selected" : ""}>Air Freight</option></select></label><label>Contacted<select name="contacted_status"><option value="">All</option>${contacted.map(x => `<option${contactedStatus === x ? " selected" : ""}>${x}</option>`).join("")}</select></label><label>Sales<select name="sales_status"><option value="">All</option>${sales.map(x => `<option${salesStatus === x ? " selected" : ""}>${x}</option>`).join("")}</select></label><label>Lead Status<select name="lead_status"><option value="">All</option>${lifecycle.map(x => `<option${leadStatus === x ? " selected" : ""}>${x}</option>`).join("")}</select></label><button>Filter</button><a href="${path}">Clear</a></form><div class="table"><table><thead><tr><th>Name</th><th>Phone</th><th>Email</th><th>Service</th><th>Cargo</th><th>Estimated Rate</th><th>Contacted</th><th>Contact Method</th><th>Sales</th><th>Lead Status</th><th>Notes</th><th>Date</th></tr></thead><tbody>${rows.map(lead => `<tr><td><a href="${path}?id=${esc(lead.id)}">${esc(lead.full_name)}</a><br><small>${esc(lead.lead_code)}</small></td><td>${esc(lead.phone)}</td><td>${esc(lead.email || "—")}</td><td>${esc(lead.service_type)}</td><td>${esc(lead.cbm)} CBM<br>${esc(lead.weight_kg)} kg${lead.density_kg_cbm === null ? "" : `<br>${Number(lead.density_kg_cbm).toFixed(2)} kg/CBM`}</td><td>${pesos(lead.estimated_rate)}</td><td>${esc(lead.contacted_status)}</td><td>${esc(lead.contact_method)}</td><td>${esc(lead.sales_status)}</td><td>${esc(lead.lead_status)}</td><td>${esc(lead.notes || "—")}</td><td>${esc(lead.created_at)}</td></tr>`).join("") || '<tr><td colspan="12">No leads found.</td></tr>'}</tbody></table></div></section>`, user.name, 200, {}, canMutateAdmin(user));
  } catch (error) {
