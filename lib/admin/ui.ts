...
export function page(
  ...
  const largerSidebarStyles = `<style>
    .admin-shell{grid-template-columns:280px minmax(0,1fr);gap:28px}
    .admin-sidebar{padding:24px}
    .admin-brand{font-size:1.3rem;margin-bottom:22px}
    .admin-nav{gap:8px}.admin-nav-group{gap:5px}
    .admin-nav-heading{font-size:.8rem;margin:18px 0 5px}
    .admin-nav-link{padding:10px 12px;font-size:1.05rem}
    .admin-nav-sub{padding:7px 12px 7px 28px;font-size:1rem}
    @media(max-width:760px){
      .admin-shell{grid-template-columns:1fr;gap:16px}
      .admin-sidebar{padding:12px}
      .admin-brand{font-size:1.1rem;margin:0}
      .admin-nav{gap:6px}
      .admin-nav-heading{font-size:.75rem;margin:14px 0 2px}
      .admin-nav-link{padding:11px 12px;font-size:1rem}
      .admin-nav-sub{padding:9px 12px 9px 28px;font-size:.95rem}
    }
  </style>`;
  body = `${largerSidebarStyles}${body}`;
  const navigation = staff
    ...
    : `<a class="admin-nav-link" href="/admin/dashboard">Dashboard</a><div class="admin-nav-heading">Customers &amp; Cargo</div><a class="admin-nav-link" href="/admin/customers">Customers</a><a class="admin-nav-link" href="/admin/shipments">Shipments</a><a class="admin-nav-link" href="/admin/quotations">Quotations</a><div class="admin-nav-heading">Billing &amp; Finance</div><a class="admin-nav-link" href="/admin/invoices">Invoices</a><div class="admin-nav-group"><a class="admin-nav-link" href="/admin/finance">Finance</a><a class="admin-nav-sub" href="/admin/finance/expenses">Expenses</a><a class="admin-nav-sub" href="/admin/finance/cash">Cash Flow &amp; Customer Credits</a></div><div class="admin-nav-heading">Pricing</div><div class="admin-nav-group"><a class="admin-nav-link" href="/admin/rates-guide">Rates &amp; Pricing</a><a class="admin-nav-sub" href="/admin/nihao-rates">Ni Hao Rates</a></div>${canWrite ? '<div class="admin-nav-heading">Admin &amp; Tools</div><a class="admin-nav-link" href="/admin/message-center">Message Center</a><a class="admin-nav-link" href="/admin/activity">Activity Log</a><a class="admin-nav-link" href="/admin/import-export">Import / Export</a><a class="admin-nav-link" href="/admin/tracking-editor">Tracking Editor</a>' : ""}<div class="admin-nav-heading">Account</div><a class="admin-nav-link" href="/cdn-cgi/access/logout">Sign Out</a>`;
  return new Response(
...
@font-face{font-family:Inter;src:url('/fonts/inter-400.woff2')}*{box-sizing:border-box}body{margin:0;background:#eef8ff;color:#154876;font:16px/1.5 Inter,Arial,sans-serif}main{max-width:1440px;margin:24px auto;padding:24px}header{display:flex;gap:20px;align-items:center;flex-wrap:wrap;margin-bottom:20px}header img{width:92px;height:72px;object-fit:contain}header p{margin:0}h1{font-size:1.8rem;line-height:1.2;margin:0 0 8px}h2{font-size:1.2rem}.admin-shell{display:grid;grid-template-columns:240px minmax(0,1fr);gap:24px}.admin-sidebar{background:#154876;border-radius:12px;padding:20px;align-self:start;position:sticky;top:20px;color:white}.admin-sidebar a{color:white;text-decoration:none}.admin-sidebar a:hover{background:#1d5b8c;text-decoration:none}.admin-brand{display:block;font-size:1.2rem;font-weight:700;margin-bottom:20px}.admin-nav{display:flex;flex-direction:column;gap:6px}.admin-nav-group{display:flex;flex-direction:column;gap:4px}.admin-nav-heading{font-size:.75rem;text-transform:uppercase;letter-spacing:.05em;color:#c6dce9;margin:16px 0 4px}.admin-nav-link{display:block;padding:8px 10px;border-radius:7px}.admin-nav-sub{display:block;padding:5px 10px 5px 24px;font-size:.9rem}.admin-content{min-width:0}.mobile-nav-checkbox,.mobile-nav-toggle{display:none}a{color:#0753ad;text-underline-offset:3px}section,.card{background:white;border:1px solid #c6dce9;border-radius:12px;padding:24px;margin-bottom:20px}.cards{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px}.finance-summary-cards{grid-template-columns:repeat(5,minmax(0,1fr))}.card{padding:20px}.card strong{display:block;font-size:1.8rem;overflow-wrap:anywhere;margin-top:12px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}label{display:block;font-size:1rem;font-weight:600}input,select,textarea{width:100%;font:inherit;color:inherit;border:1px solid #94b3c7;border-radius:6px;padding:10px;margin-top:6px;background:white}textarea{min-height:100px;resize:vertical}input:focus,select:focus,textarea:focus,button:focus,a:focus{outline:3px solid #0099cf;outline-offset:3px}button,.button{display:inline-block;border:0;background:#0753ad;color:white;font:inherit;border-radius:7px;padding:11px 20px;cursor:pointer;text-decoration:none}.actions{display:flex;flex-wrap:wrap;align-items:center;gap:18px;margin-top:20px}.search{display:flex;align-items:end;gap:16px;flex-wrap:wrap}.search label{flex:1;min-width:170px}.notice{background:#e7f4fc;border:1px solid #87afc6;padding:16px;border-radius:8px;overflow-wrap:anywhere}.table{overflow:auto;-webkit-overflow-scrolling:touch}table{width:100%;border-collapse:collapse;text-align:left}th,td{padding:14px 10px;border-bottom:1px solid #d5e4ed;vertical-align:top;overflow-wrap:anywhere}th{font-size:.9rem}dl{display:grid;grid-template-columns:180px minmax(0,1fr);gap:12px}dt{font-weight:bold}dd{margin:0;white-space:pre-wrap;overflow-wrap:anywhere}.muted{color:#466177}.wide{grid-column:1/-1}@media(max-width:760px){main{padding:14px;margin:0 auto}header{gap:12px;margin:8px 0 16px}header img{width:72px;height:56px}h1{font-size:1.55rem}.admin-shell{grid-template-columns:1fr;gap:16px}.admin-sidebar{position:static;padding:12px;border-radius:10px}.admin-brand{display:inline-block;margin:0;font-size:1.1rem}.mobile-nav-toggle{display:block;cursor:pointer;font-weight:700;margin-top:10px;padding:11px 12px;border:1px solid #87afc6;border-radius:7px;min-height:44px}.admin-nav{display:none;margin-top:10px;padding-top:8px;border-top:1px solid #4d7899}.mobile-nav-checkbox:checked~.admin-nav{display:flex}.admin-nav-link{padding:11px 12px;min-height:44px}.admin-nav-sub{padding:9px 12px 9px 28px;min-height:40px}.admin-nav-heading{margin:14px 0 2px}.cards,.grid,.finance-summary-cards{grid-template-columns:1fr}section,.card{padding:18px}.actions{align-items:stretch;gap:10px}.actions button,.actions .button{min-height:44px}.search{gap:10px}.search label{min-width:100%}dl{grid-template-columns:1fr;gap:5px}dd{margin-bottom:12px}}@media(max-width:520px){main{padding:10px}header img{width:62px;height:48px}h1{font-size:1.4rem}section,.card{padding:14px}.actions button,.actions .button,.search button{width:100%}th,td{padding:11px 8px}}
