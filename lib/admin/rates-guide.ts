...
<section id="sea-freight"><h2>1. Sea Freight — Rates &amp; Package Tiers</h2><h3>Item, Category &amp; Rates</h3><form method="get" class="search"><label>Search items<input name="q" value="${esc(url.searchParams.get("q") ?? "")}" placeholder="Item or category"></label><label>Category<select name="category"><option value="">All categories</option>${categories.map(category => `<option value="${esc(category)}"${selectedCategory === category ? " selected" : ""}>${esc(category)}</option>`).join("")}</select></label><button>Filter</button><a href="/admin/rates-guide">Clear</a></form><div class="table"><table><thead><tr><th>ITEM</th><th>CATEGORY</th><th>CBM RATE</th><th>DENSITY RATE</th></tr></thead><tbody>${rows}</tbody></table></div><h3>Package Tiers</h3><div class="table"><table><thead><tr><th>PACKAGE TIER</th><th>CBM / DENSITY</th><th>BASE RATE</th><th>CALCULATION RULE</th></tr></thead><tbody><tr><td>KD Mini</td><td>≤0.01 CBM</td><td>₱250</td><td>If density &gt;425, compare ₱250 with weight × category density rate. Higher wins.</td></tr><tr><td>KD Lite</td><td>&gt;0.01–0.05 CBM</td><td>₱750</td><td>If density &gt;425, compare ₱750 with weight × category density rate. Higher wins.</td></tr><tr><td>KD Plus</td><td>&gt;0.05–0.125 CBM</td><td>₱1,400</td><td>If density &gt;425, compare ₱1,400 with weight × category density rate. Higher wins.</td></tr><tr><td>KD Standard</td><td>&gt;0.125 CBM</td><td>Minimum ₱1,700</td><td>Base = MAX(₱1,700, CBM × category CBM rate). If density &gt;425, compare base with density charge.</td></tr><tr><td>KD Max</td><td>Density charge wins</td><td>Density-based charge</td><td>Applies only when density is &gt;425 kg/CBM and the density charge is higher than the base charge.</td></tr></tbody></table></div><p class="notice">Mobile / Computers / Tablets are exempt from density pricing and KD Max.</p></section>
