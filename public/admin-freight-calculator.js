(() => {
  const config = document.currentScript || document.querySelector("script[data-sea-items][src$='admin-freight-calculator.js']");
  if (!config) return;
  const seaItems = JSON.parse(config.dataset.seaItems || "{}");
  const airItems = JSON.parse(config.dataset.airItems || "[]");
  ...
  const type = form.querySelector("[data-freight-type]"), item = form.querySelector("[data-item]"), category = form.querySelector("[data-category]"), cbm = form.querySelector("[data-cbm]"), weight = form.querySelector("[data-weight]"), units = form.querySelector("[data-units]"), unitsField = form.querySelector("[data-units-field]"), csrf = form.querySelector("[data-csrf]"), error = form.querySelector("[data-calculator-error]"), sea = document.querySelector("[data-sea-results]"), air = document.querySelector("[data-air-results]");
  ...
  form.addEventListener("submit", async (event) => {
    ...
    } catch (cause) { showError(cause instanceof Error ? cause.message : "Unable to calculate freight."); }
  });
  type.addEventListener("change", sync); item.addEventListener("change", sync); sync();
