...
type Estimate = { packageName: string; amount: number };
...
export default function RatesCalculatorPage() {
  ...
  const density = useMemo(() => {
    ...
    if (service !== "sea" || !Number.isFinite(numericCbm) || !Number.isFinite(numericWeight) || numericCbm <= 0 || numericWeight <= 0) return null;
  ...
  const calculate = (event: FormEvent<HTMLFormElement>) => {
    ...
    setEstimate(getAirEstimate(numericWeight));
    setError("");
    trackEvent("calculate_shipping");
  };
                  ...
                  <>
                    <div className="kd-calculator-input-row">
                      <label>
                        ...
                        <input aria-label="CBM cubic meter" type="number" min="0" step="any" inputMode="decimal" placeholder="0.05" value={cbm} onChange={(event) => { setCbm(event.target.value); setEstimate(null); setError(""); }} />
                      </label>
                      <label>
                        <span>WEIGHT (KG)</span>
                        <input aria-label="Weight kilograms" type="number" min="0" step="any" inputMode="decimal" placeholder="100" value={weight} onChange={(event) => { setWeight(event.target.value); setEstimate(null); setError(""); }} />
                      </label>
                    </div>
                    <label className="kd-density-field">
                      <span>DENSITY (KG/CBM)</span>
                      <input aria-label="Calculated density kilograms per cubic meter" type="text" readOnly value={density === null ? "" : `${density.toLocaleString("en-PH", { maximumFractionDigits: 2 })} kg/CBM`} placeholder="Calculated automatically" />
                    </label>
                    <p className="kd-calculator-helper"><Info aria-hidden="true" /> Both CBM and weight are required for Sea Freight.</p>
                  </>
                ) : (
                  <>
                    <label>
              ...
              <section className="kd-result-card" aria-live="polite" aria-labelledby="estimate-title">
                ...
                {!estimate ? (
                  ...
                  <div className="kd-result-filled">
                    <ResultRow label="PACKAGE" value={estimate.packageName} />
                    <div className="kd-result-total"><span>ESTIMATED ALL-IN RATE</span><strong>{peso(estimate.amount)}</strong></div>
                  </div>
                )}
              </section>
            </div>
            <aside className="kd-rates-disclaimer"><ShieldCheck aria-hidden="true" /><div><p>Disclaimer: The calculated shipping fee is an estimate only and is provided for reference. Final shipping charges are subject to actual weight, dimensions, CBM, cargo type, density, warehouse measurement, and other applicable shipping requirements.</p><p>Density Formula: Weight (kg) ÷ CBM = Density (kg/CBM)</p><p>Example: 100 kg ÷ 0.50 CBM = 200 kg/CBM</p></div></aside>
          </div>
        </section>
