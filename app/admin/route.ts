...
<p>
  Add a shipment, or load its cargo code to update the details.
</p>
...
>
  <label>
    ...
    <input
      ...
      required
      maxlength="100"
      placeholder="KDOOR-0001"
      value="${escape(values.cargo_code)}"
    >
  </label>
  ...
  >
    ${["Sea Freight", "Air Freight"]
      .map(
        (v) =>
          `<option${
            values.freight_type === v ? " selected" : ""
          }>${v}</option>`,
      )
      .join("")}
  </select>
...
${input("origin_warehouse", "Origin Warehouse")}
...
<label>
  ...
  <input
    ...
    list="statuses"
    value="${escape(
      values.current_status ?? "Received at Warehouse",
    )}"
  >
...
<p class="help">
  * Required. Use KDOOR-0001 for sea freight or
  AIR-KDOOR-0001 for air freight.
  Update replaces the details for this cargo code;
  load it first. Blank optional fields are cleared.
</p>
 ...
 *
 * The previous HTTP Basic Authentication layer has
 * intentionally been removed so users do not receive
 * a second browser username/password prompt.
 */
...
export async function GET(request: Request) {
  try {
    ...
    const shipment = await env.TRACKING_DB
      .prepare(
        `SELECT ${fields.join(", ")}
         FROM shipments
         WHERE cargo_code = ?
    ...
    if (!shipment) {
      return page(
        { cargo_code: code },
        "Shipment not found. Check the cargo code or add a new shipment.",
        404,
      );
    }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    /*
     * Protect same-origin form submissions.
     *
     * Safari may submit same-page HTML forms
     * with Origin: null.
     */
    ...
    if (action === "add") {
      /*
       * Atomic conditional insert also handles
    ...
    return response(
      "Could not save the shipment. Return to /admin and load the cargo code to check its current details before retrying.",
      500,
    );
  }
}
